...
class CurrencyConverter {
    ...
    async loadRatesFromAdmin() {
        ...
        try {
            ...
            const ratesSnap = await get(ref(this.db, 'zelen/currencies'));
            if (ratesSnap.exists()) {
                ...
                this.rates = { EUR: 1.0 };
                await set(ref(this.db, 'zelen/currencies'), this.rates);
            }
        } catch (error) {
    ...
    async loadConversionTax() {
        ...
        try {
            ...
            const taxSnap = await get(ref(this.db, 'zelen/conversionTax'));
            if (taxSnap.exists()) {
    ...
    async transferToReserve(amount, currency, userId) {
        ...
        try {
            ...
            const reserveSnap = await get(ref(this.db, 'zelen/accounts/BANK_RESERVE/accounts'));
            let accounts = {};
            ...
            if (!reserveAccount) {
                const newAccountId = push(ref(this.db, 'zelen/accounts/BANK_RESERVE/accounts')).key;
                reserveAccount = {
                    currency: currency,
                    balance: 0,
                ...
                await set(ref(this.db, 'zelen/accounts/BANK_RESERVE/accounts'), accounts);
                console.log(`Создан резервный счет для валюты ${currency}`);
            }
            
            // Пополняем резервный счет
            await update(ref(this.db, `zelen/accounts/BANK_RESERVE/accounts/${reserveAccountId}`), {
                balance: reserveAccount.balance + amount
            });
            
            // Записываем транзакцию
            const transactionId = push(ref(this.db, `zelen/accounts/BANK_RESERVE/accounts/${reserveAccountId}/transactions`)).key;
            await set(ref(this.db, `zelen/accounts/BANK_RESERVE/accounts/${reserveAccountId}/transactions/${transactionId}`), {
                type: 'conversion_fee',
                amount: amount,
                currency: currency,
